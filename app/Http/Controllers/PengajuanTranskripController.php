...
use App\Http\Requests\StorePengajuanTranskripRequest;
use App\Http\Requests\UpdatePengajuanTranskripRequest;
...
class PengajuanTranskripController extends Controller
{
  // Menampilkan daftar pengajuan (untuk admin & mahasiswa)
    ...
    {
        return view('students.pengajuan.form');
    }
    ...
    {
        return view('pengajuan-transkrip.edit', compact('pengajuan'));
    }
    ...
    public function downloadTranskrip()
    {
        ...
        if ($khs->isEmpty() || $khs->contains(fn($item) => $item->khs === null)) {
            ...
            // Generate PDF
            $pdf = PDF::loadView('students.pengajuan.pdf', compact(
