...
class TagihanMahasiswaController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('admin.keuangan.tagihan-mahasiswa.index', compact('tagihan', 'programStudiList', 'tahunMasukList', 'mahasiswaList', 'programStudiId', 'tahunMasuk', 'mahasiswaId'));
    }
    ...
    public function create(): View
    {
        ...
        return view('admin.keuangan.tagihan-mahasiswa.form', compact('programStudi', 'tenor', 'mahasiswa', 'tahunAjaran'));
    }
    ...
    public function edit($id)
    {
        ...
        return view('admin.keuangan.tagihan-mahasiswa.form', compact('tagihan', 'programStudi', 'tenor', 'mahasiswa', 'tahunAjaran'));
    }
    ...
    public function generateTagihan()
    {
        ...
        // Proses menggunakan chunk untuk menghindari pemborosan RAM jika ada ribuan mahasiswa
        Mahasiswa::where('status_mhs', 'aktif')->chunk(200, function ($mahasiswas) use ($tahunAjaranAktif, &$totalTagihanBaru) {
            $tagihanData = [];

            // Pre-load data Tenor dan Tarif untuk meminimalisir N+1 Query
            // Mengingat kombinasi angkatan & jurusan banyak, kita masih query per group atau per mahasiswa tetapi dengan optimasi
            foreach ($mahasiswas as $mahasiswa) {
                ...
                if (!$tarif) {
                    continue; // Skip jika belum ada setting tarif untuk angkatan ini
                }

                // Ambil tenor pembayaran khusus untuk angkatan dan gelombang ini
                ...
                if ($tenorList->isEmpty()) {
                    continue; // Skip jika belum ada setting tenor
                }
                ...
                foreach ($tenorList as $tenor) {
                    ...
                    // [FIX] Perhitungan Matematika Nominal Tagihan (Berdasarkan Persentase)
   ...
   public function searchTagihan(Request $request)
    {
        ...
        $query = TagihanMahasiswa::query()
            ->with('tenorPembayaran', 'mahasiswa', 'transaksi') // Eager load untuk menghindari N+1 Query Problem
   ...
   public function getMahasiswa(Request $request)
    ...
    public function updatePembayaran(Request $request, $id)
    {
        ...
        \App\Models\TransaksiPembayaran::create([
            ...
            'status_verifikasi' => 'diterima', // Karena diinput langsung oleh TU
        ]);

        // 2. Hitung total yang sudah dibayar
        $totalTelahDibayar = \App\Models\TransaksiPembayaran::where('tagihan_mahasiswa_id', $tagihan->id)
        ...
        // Simpan total pembayaran di tabel tagihan jika kolom exist (optional fail-safe)
        try {
        ...
        return response()->json(['success' => true, 'message' => 'Pembayaran otomatis tercatat ke histori transaksi.', 'sisa' => ($tagihan->jumlah_tagihan - $totalTelahDibayar)]);
    }
