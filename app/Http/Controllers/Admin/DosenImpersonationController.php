...
class DosenImpersonationController extends Controller
{
    public function store(Request $request, Dosen $dosen): RedirectResponse
    {
        ...
        if ($request->session()->get('impersonating_mahasiswa')) {
            ...
            if ($mahasiswa) {
                activity_log_for(
                    ...
                    'Admin berpindah dari akun mahasiswa '.$mahasiswa->nama.' ke akun dosen '.$dosen->nama
                );
                activity_log_for(
                    $mahasiswa,
                    'mahasiswa',
                    'impersonated_logout',
                    'Sesi impersonasi mahasiswa dihentikan karena admin berpindah ke akun dosen.'
                );
            }
