            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select name="semester" class="form-control" required>
                        ...
                        @for ($i = 1; $i <= 10; $i++)
                            <option value="{{ $i }}" {{ old('semester', $tenor->semester ?? '') == $i ? 'selected' : '' }}>
                                Semester {{ $i }}
                            </option>
                        @endfor
                    </select>
                ...
                <div class="col-md-6 mb-3">
                    <label for="tahun_masuk" class="form-label"><strong>Tahun Angkatan (Masuk):</strong></label>
                    <select name="tahun_masuk" class="form-control" required>
                        <option value="">-- Pilih Tahun Angkatan --</option>
                        @php $currentYear = date('Y') + 1; @endphp
                        @for ($year = $currentYear; $year >= 2015; $year--)
                            <option value="{{ $year }}" {{ old('tahun_masuk', $tenor->tahun_masuk ?? '') == $year ? 'selected' : '' }}>
                                {{ $year }}
                            </option>
                        @endfor
                    </select>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="gelombang_id" class="form-control" required>
                        ...
                        @foreach($gelombang as $gel)
                            <option value="{{ $gel->id }}" {{ old('gelombang_id', $tenor->gelombang_id ?? '') == $gel->id ? 'selected' : '' }}>
                                {{ $gel->nama }}
                            </option>
                        @endforeach
                    </select>
