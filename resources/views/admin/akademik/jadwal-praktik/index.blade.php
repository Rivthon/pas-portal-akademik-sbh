...
<div class="card shadow-sm mb-4 border-top border-5 border-success">
    <div class="card-header bg-white pb-0 d-flex justify-content-between align-items-center">
        <div>
            <h5 class="card-title text-success mb-0"><i class="bx bx-import me-1"></i> Tarik Data Jadwal Praktik</h5>
            <small class="text-muted">Generate data awal jadwal praktik berdasarkan kurikulum yang memiliki sesi praktik.</small>
        </div>
    </div>
...
<div class="card shadow-sm mt-4 border-top border-5 border-primary">
    <div class="card-header bg-white pb-0">
        <h5 class="card-title text-primary mb-0"><i class="bx bx-search me-1"></i> Cari & Edit Jadwal Praktikum</h5>
    </div>
    <div class="card-body">
        <div class="row mt-3">
...
<div class="card mt-4 mb-5">
    <div class="card-body">
        <h5 class="card-title"><i class="bx bx-table me-1"></i> List Jadwal Praktik</h5>
        <div id="loading" class="text-center my-4" style="display: none;">
            <div class="spinner-border text-primary" role="status">
                <span class="visually-hidden">Loading...</span>
            </div>
            <div class="mt-2 text-muted">Memuat data jadwal praktik...</div>
        </div>
...
<script>
    document.getElementById('search-btn').addEventListener('click', function () {
        ...
        fetch(`{{ route('admin.jadwal-praktik.filter') }}?programStudi=${programStudi}&semester=${semester}&jenis_kelas=${jenis_kelas}`)
            .then(async response => {
            ...
            .then(({ ok, status, data }) => {
                    ...
                    if (status === 404 && data.message) {
                        alertContainer.innerHTML = `
                            <div class="alert alert-warning border border-warning alert-dismissible" role="alert">
                                ...
                                <p class="mb-0">${data.message} <br>Anda dapat menggunakan form <strong>Tarik Data Jadwal Praktik</strong> di atas untuk men-generate jadwal secara otomatis.</p>
                ...
                if (data.length === 0) {
                    alertContainer.innerHTML = `<div class="alert alert-info"><i class="bx bx-info-circle me-1"></i> Tidak ada jadwal praktik yang ditemukan.</div>`;
                    return;
                }
            ...
            .catch(error => {
                loading.style.display = "none";
                alertContainer.innerHTML = `<div class="alert alert-danger"><i class="bx bx-x-circle me-1"></i> Terjadi kesalahan saat mengambil data jadwal praktik.</div>`;
    ...
    document.addEventListener('click', function (e) {
        ...
        if (btn) {
            ...
            Swal.fire({
                title: "Hapus Jadwal?",
                text: "Jadwal praktik ini akan dihapus secara permanen!",
                icon: "warning",
                ...
                if (result.isConfirmed) {
                    deleteJadwalPraktik(id, row);
                }
            });
        }
    });

    function deleteJadwalPraktik(id, row) {
        fetch(`/admin/jadwal-praktik/delete/${id}`, {
            method: 'DELETE',
            headers: {
                'X-Requested-With': 'XMLHttpRequest',
                'X-CSRF-TOKEN': '{{ csrf_token() }}' // Fix CSRF Token using Blade Helper instead of querySelector
            }
        })
    ...
    if (document.getElementById('submitBtngenerate')) {
        document.getElementById('submitBtngenerate').addEventListener('click', function () {
            ...
            Swal.fire({
                ...
                text: "Sistem akan me-generate form jadwal praktik secara otomatis berdasarkan kurikulum.",
                icon: 'question',
                showCancelButton: true,
