        ...
        <div class="row g-0 align-items-center">
            <div class="col-md-7">
                ...
                <p class="mb-4 text-muted" style="line-height: 1.6;">
                    Halaman ini berisi daftar jadwal Ujian Akhir Semester (UAS) mahasiswa. Atur tanggal, ruangan, dan jam pelaksanaan dengan mudah.
...
<script>
    document.getElementById('search-btn').addEventListener('click', function () {
        ...
        fetch(`{{ route('admin.jadwal-uas.filter') }}?programStudi=${programStudi}&semester=${semester}&jenis_kelas=${jenis_kelas}`)
            ...
            .catch(error => {
                loading.style.display = "none";
                alertContainer.innerHTML = `<div class="alert alert-danger"><i class="bx bx-x-circle me-1"></i> Terjadi kesalahan saat mengambil data jadwal UAS.</div>`;
    ...
    async function updateJadwalAjax(id, field, value, element, originalBg) {
        try {
            let response = await fetch(`{{ url('/admin/jadwal-uas/update') }}/${id}`, {
                method: 'POST',
                headers: {
