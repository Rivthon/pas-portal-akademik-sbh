...
@section('title', isset($jadwalUts) ? 'Edit Jadwal UTS' : 'Tambah Tambah Jadwal UTS')
@section('content')
<div class="container mt-5">
    ...
    <div class="card">
        <div class="card-header">
            <h5>{{ isset($jadwalUts) ? 'Update Jadwal UTS' : 'Tambah Jadwal UTS' }}</h5>
        </div>
        <div class="card-body">
            <form
                action="{{ isset($jadwalUts) ? route('admin.jadwal-uts.update', $jadwalUts->id) : route('admin.jadwal-uts.store') }}"
                method="POST">
                @csrf
                @if(isset($jadwalUts))
                @method('PUT')
                ...
                @if(isset($jadwalUts))
                <input type="hidden" name="id" value="{{ $jadwalUts->id }}">
                @endif
                ...
                <div class="d-flex justify-content-between mt-3">
                    <a href="{{ route('admin.jadwal-uts.index') }}" class="btn btn-secondary">Kembali</a>
                    <button type="submit" class="btn btn-primary"
                        onclick="return confirm('Apakah Anda yakin ingin menyimpan data ini?')">
                        {{ isset($jadwalUts) ? 'Perbaharui' : 'Simpan' }}
                    </button>
                </div>
            </form>
        </div>
